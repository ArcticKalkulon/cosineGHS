...
def main():
	...
	############################
	# SETUP
    # set up 2D integration for different systems
    # mu = cos(theta) integration from 0 to 1
    #
    ...
    for n in range(N):
        ...
        Hl = OTF_incident_beam(Rlam,rrad)
        G = G*Hl
        Srad = ht.fht(G)
...
def OTF_incident_beam(R,r):
    return_array = np.zeros(r.shape)
    temp_r = r[r<2*R]
    return_array[r<2*R] = 2/np.pi*(np.arccos(temp_r/2/R)-temp_r/2*np.sqrt(1-temp_r**2/4/R**2))
    return return_array
